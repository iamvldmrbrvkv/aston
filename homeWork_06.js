...
setTimeout(() => console.log("2"), 1); // 5. попадает в стек, колбек улетает в очеред макрозадач и выполнится сразу после микрозадач
let promiseNew = new Promise((resolve) => {
  console.log("3"); // 2. попадает в стек с сразу выполняется, лог 3
  resolve(); // синхронно выполняется функция resolve
});
promiseNew.then(() => console.log("4")); // 4, колбек в then улетает в очередь микрозадач и выполнится сразу как будет пустой стек
setTimeout(() => console.log("5")); // 6. колбек улетает в очеред макрозадач и выполнится после микрозадач если стек будет пуст
...
let promiseTree = new Promise((resolve, reject) => {
  ...
  setTimeout(() => {
    console.log("2"); // 3, колбек улетает в очередь макрозадач и выполнится когда стек будет пуст
  }, 0);
...
doSmth() // функциия возвращает успешновыполеннный промис со значением 123
  .then(function (a) {
    ...
    return a; // возвращаем
  }) // метод then вернул успешновыполеннный промис со значением 123, так как в колбеке мы никак не манипулировали этим значением
  .then(function (b) {
    // обрабатываем значение 123 успешновыполненного промиса
...
// так как у нас есть микрозадачи и макрозадачи, то первыми в стек попадает пачка функий из микрозадач и уже потом по одной в стек попадают колбеки макрозадач, колбеки setTimeout
...
new Promise((res, rej) => {
  ...
  .then(console.log("b 1")) // в методе then не функция, поэтому код then просто выполняется не влияя никак на дальнейшую цепочку, тем не менее результатом этого then помимо вывода в консоль значения будет и отклоненный промис со значением 123, просто идет передача отклоненного промиса со значением 123 туда вперед
  .then( // вызываем метод then на отклоненном промисе со значением 123
